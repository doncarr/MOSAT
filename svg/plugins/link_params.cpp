...
class link_params_t : public gen_plugin_base_t
{
public:
...
{
  return new link_params_t;
}

const char *link_params_t::get_name(void)
{
  return "link_params";
}

void link_params_t::generate_doc(doc_object_base_t *dob)
{
  dob->start("link_params", "Multi-line link to load other web page");
  ...
  dob->example("link_params|100|12|6|firebrick|middle|cool.svg|Cooling Tower|feed.svg|Feed Tanks|");
  dob->notes("Additional links appear to the right of the first with a separating '|'.");
  dob->end();
}

void link_params_t::generate(plugin_data_t d, int argc, char **argv)
{
  if (argc < 8)
  {
    printf("%s, line %d: There must be AT LEAST 8 arguments to link_params\n",
  ...
  { 
    ...
    fprintf(d.svg_fp, "  <tspan onclick=\"open_with_params('%s');\"\n"
                      "     onmouseover=\"this.style.cursor='pointer';\">%s</tspan>\n", 
             the_link, the_text);
  }
  fprintf(d.svg_fp, "</text>\n");
  fprintf(d.svg_fp, "<!--  END insert for link_params (%03d) -->\n", n_instance);
  add_js_library("open_with_params.js");
  n_instance++;
}
